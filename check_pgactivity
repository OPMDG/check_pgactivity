...
my $PG_VERSION_150 = 150000;
my $PG_VERSION_160 = 160000;
my $PG_VERSION_170 = 170000;
my $PG_VERSION_180 = 180000;
...
my %services = (
    ...
    'slru_hit_ratio' => {
        'sub'  => \&check_slru_hit_ratio,
        'desc' => 'Check SLRU ratio on a host.'
    },
);
...
# Record on disk some given data to a given file.
#
# To avoid data loss, the sub:
# * first require an exclusive lock on the lock file,
# * load the existing data
# * call the closure to record its data in there
# * write back the edited data to disk
# * close the file and release the lock.
#
# The closure is reponsible to define how and where in the structure it wants to
# record its data.
#
# Parameters are :
#  * $fn_record: a closure responsible to record its data in the given
#                structure ref.
#  * $storage: file to write to.
sub save_internal($$) {
    my $fn_record = shift;
    my $storage   = shift;
    my $all       = {};
    ...
    if ( -e $storage ) {
        ...
        die "can not write to binary file «${storage}»" unless -w ${storage};
        die "file «${storage}» not recognized as a check_pgactivity binary file"
    ...
    # closing the fh releases the lock
...
sub save($$$$) {
    ...
    save_internal($fn_record, $storage);
}

# Lock, load, unlock and return the whole content of the status file on disk.
sub load_all($) {
    my $storage  = shift;
    my $lockfile = "${storage}.lock";
    my $all;
...
v10+: grant execute on function pg_stat_file(text, bool).
    ...
    if (check_compat $hosts[0], $PG_VERSION_81, $PG_VERSION_96) {
        ...
        if ( $nb_files > 0 ) {
            ...
            if ( $rs[0][0] ne $prev_archiving ) {
                ...
                push @msg => sprintf 'archiver failing on %s', substr($rs[0][0], 0, 24);
        ...
        # field 1: number of WAL segment to archive
        # field 2: how long archiving is failing in second
        # field 3: last segment archived
        # field 4: last failing segment
        # field 5: mod time of the next wal to archive (only for superuser in v10)
        my %queries = (
        $PG_VERSION_100 => q{
        SELECT coalesce(pg_wal_lsn_diff(
                    current_pos,
                    /* compute LSN from last archived offset */
                    (to_hex(COALESCE(last_archived_off, last_failed_off)/4294967296)
                    ||'/'||to_hex(COALESCE(last_archived_off, last_failed_off)%4294967296))::pg_lsn
                )::bigint / walsegsize, 0),
                failing,
                CASE WHEN failing
        ...
        FROM (
            SELECT last_archived_wal, last_archived_time, last_failed_wal,
                ...
                -- WAL offset
                ('x'||substr(last_archived_wal_hex, 9, 8))::bit(32)::bigint*4294967296
                ...
                ('x'||substr(last_failed_wal_hex, 9, 8))::bit(32)::bigint*4294967296
                    -- offset to the begining of the segment
                    + ('x'||substr(last_failed_wal, 17, 8))::bit(32)::bigint * walsegsize
                    -- offset to the end of the segment
                    + walsegsize AS last_failed_off,
                ...
                (last_failed_time >= last_archived_time)
                    ...
                    AS failing
              FROM (SELECT last_archived_wal,
                last_archived_time,
                CASE WHEN (last_archived_wal NOT LIKE '%.history' AND last_archived_wal NOT LIKE '%.backup')
                    THEN last_archived_wal
                    ELSE NULL -- return NULL if last successfully archived file is a .history or .backup file
                END AS last_archived_wal_hex,
                CASE WHEN (last_failed_wal NOT LIKE '%.history' AND last_archived_wal NOT LIKE '%.backup')
                    THEN last_failed_wal
                    ELSE NULL -- return NULL if last failed archive is a .history or .backup file
                END AS last_failed_wal_hex,
                last_failed_wal,
                last_failed_time
        ...
        if ( $rs[0][1] eq 't' ) {
            push @msg => sprintf 'archiver failing on %s', $rs[0][4];
            if ( $rs[0][2] ne '' ) {
                push @longmsg => sprintf '%s not archived since %ds',
                                $rs[0][4], $rs[0][2];
            }
        else {
                push @longmsg => sprintf '%s not archived',
                                $rs[0][4];
            }
        }
        ...
        if ( $nb_files > 0 ) {
            if ( $rs[0][5] ne '' ) {
                push @perfdata => [ 'oldest_ready_wal', int( $rs[0][5] ), 's',
                                undef, undef, 0 ];
            }
        }
        else {
...
sub check_checksum_errors {
    ...
    pod2usage(
        -message => 'FATAL: you must give only one host with service "checksum_errors".',
        -exitval => 127
    ) if @hosts != 1;

    is_compat $hosts[0], 'checksum_errors', $PG_VERSION_120 or exit 1;
...
sub check_session_stats {
    ...
    my %sstats;
    my %new_sstats;
    my $instance_session_rate;
    my %args  = %{ $_[0] };
    my $me    = 'POSTGRES_SESSION_STATS';
    my $sql   = q{
        SELECT extract(EPOCH from now()), s.datname, s.sessions,
            s.session_time, s.active_time, s.idle_in_transaction_time,
            s.sessions_killed, s.sessions_abandoned, s.sessions_fatal
    ...
    pod2usage(
        -message => 'FATAL: you must give only one host with service "session_stats".',
        -exitval => 127
    ) if @hosts != 1;

    is_compat $hosts[0], 'session_stats', $PG_VERSION_140 or exit 1;

    %sstats = %{ load( $hosts[0], 'session_stats', $args{'status-file'} ) || {} };
    ...
    $new_sstats{$_->[1]} = {
        ...
        'session_time'      => $_->[3],
        'active_time'       => $_->[4],
        'idle_in_xact_time' => $_->[5],
        'killd'             => $_->[6],
        'aband'             => $_->[7],
        'fatal'             => $_->[8]
    } foreach @rs;

    save $hosts[0], 'session_stats', \%new_sstats, $args{'status-file'};

    return status_ok( $me, ['First call'] ) unless keys %sstats;

    foreach my $db ( keys %new_sstats ) {

        my $sessions = $new_sstats{$db}{'sessions'} - $sstats{$db}{'sessions'};
        my $session_time = $new_sstats{$db}{'session_time'} - $sstats{$db}{'session_time'};
        my $active_time  = $new_sstats{$db}{'active_time'}  - $sstats{$db}{'active_time'};
        my $idle_in_xact_time  = $new_sstats{$db}{'idle_in_xact_time'}  - $sstats{$db}{'idle_in_xact_time'};

        my $sessions_killd  = $new_sstats{$db}{'killd'}  - $sstats{$db}{'killd'};
        my $sessions_aband  = $new_sstats{$db}{'aband'}  - $sstats{$db}{'aband'};
        my $sessions_fatal  = $new_sstats{$db}{'fatal'}  - $sstats{$db}{'fatal'};

        my $session_rate = $sessions / ( $new_sstats{$db}{'ts'} - $sstats{$db}{'ts'} );
        $instance_session_rate += $session_rate;

        push @perfdata => (
            [ "${db}_session_rate", $session_rate, ' sessions/s' ],

            [ "${db}_session_time", $session_time, 'ms' ],
            [ "${db}_active_time",  $active_time,  'ms' ],
            [ "${db}_idle_in_transaction_time", $idle_in_xact_time, 'ms' ],

            [ "${db}_sessions_killed",    $sessions_killd ],
            [ "${db}_sessions_abandoned", $sessions_aband ],
            [ "${db}_sessions_fatal",     $sessions_fatal ]
        );
    }

    return status_ok(
        $me,
        ["Number of sessions per second for all databases: " . sprintf( "%.2f", $instance_session_rate )],
        \@perfdata );
}
...
sub check_database_size {
    ...
    if ( defined $args{'warning'} ) {
        my $thresholds_re = qr/(size|delta)\s*=\s*([^,]+)/i;
...
=item B<slru_hit_ratio> (all)

Check the cache SLRU ratio on the cluster.

This service uses the status file (see C<--status-file> parameter).

Perfdata returns the cache SLRU ratio for one cluster.
...
sub check_slru_hit_ratio {
    ...
    my @hosts;
    my %slru_hitratio;
    my %new_slru_hitratio;
    my %args       = %{ $_[0] };
    my $me         = 'POSTGRES_SLRU_HIT_RATIO';
    my $slru_checked = 0;
    my $sql        = q{SELECT name, blks_hit, blks_read
        FROM pg_stat_slru
        ORDER BY name};
    ...
    pod2usage(
        -message => 'FATAL: you must give only one host with service "slru_hit_ratio".',
        -exitval => 127
    ) if @hosts != 1;


    %slru_hitratio = %{ load( $hosts[0], 'slru_hitratio', $args{'status-file'} ) || {} };

    @rs = @{ query( $hosts[0], $sql ) };

    SLRU_LOOP: foreach my $slru (@rs) {
        my $ratio;
        my $hit_delta;
        my $read_delta;
        my @perfdata_value;

        $new_slru_hitratio{ $slru->[0] } = [ $slru->[1], $slru->[2], 'NaN' ];

        $slru_checked++;

        next SLRU_LOOP unless defined $slru_hitratio{ $slru->[0] };

        $hit_delta  = $new_slru_hitratio{ $slru->[0] }[0] - $slru_hitratio{ $slru->[0] }[0];
        $read_delta = $new_slru_hitratio{ $slru->[0] }[1] - $slru_hitratio{ $slru->[0] }[1];
        ...
        if ( $hit_delta + $read_delta > 0 ) {
            ...
            # rounding the fractional part to 2 digits
            $ratio = int($ratio*100+0.5)/100;
            $new_slru_hitratio{ $slru->[0] }[2] = $ratio;

            @perfdata_value = ( $slru->[0], $ratio, '%' );
        }
        # Without activity since last run, use previous hit ratio.
        # This should not happen as the query itself hits/reads.
        elsif ( $slru->[1] + $slru->[2] > 0 ) {
            $ratio = $slru_hitratio{ $slru->[0] }[2];
            @perfdata_value = ( $slru->[0], $ratio, '%' );
        }
        # This database has no reported activity yet
        else {
            $ratio='NaN';
            $new_slru_hitratio{ $slru->[0] }[2] = 'NaN';
            @perfdata_value = ( $slru->[0], 'NaN', '%' );
        }
        ...
        if ( defined $args{'critical'} ) {
            if ( $ratio < $args{'critical'} ) {
                push @msg_crit => sprintf "%s: %s%%", $slru->[0], $ratio;
                next SLRU_LOOP;
            }

            if ( defined $args{'warning'} and $ratio < $args{'warning'} ) {
                push @msg_warn => sprintf "%s: %s%%", $slru->[0], $ratio;
            }
        }
    }

    save $hosts[0], 'slru_hitratio', \%new_slru_hitratio, $args{'status-file'};
    ...
    return status_ok( $me, [ "$slru_checked SLRU(s) checked" ], \@perfdata );
}
...
sub check_oldest_idlexact {
    ...
    my %queries     = (
        ...
        $PG_VERSION_94 => q{SELECT d.datname,
            coalesce(extract('epoch' FROM
                    date_trunc('second', current_timestamp-state_change)
                ), -1)
            FROM pg_database AS d
            LEFT JOIN pg_stat_activity AS a ON (a.datid = d.oid AND a.state='idle in transaction'
	            AND NOT (a.backend_xid IS NULL AND a.backend_xmin IS NULL))
        }
    );

    # Exclude some apps
    if(defined $args{'exclude'}[0]){
        $exclude_filter = " WHERE a.application_name NOT IN ('".join("','", split(',', $args{'exclude'}[0]))."')";
        $queries{$PG_VERSION_92} .= $exclude_filter;
        $queries{$PG_VERSION_94} .= $exclude_filter;
    }
...
Too many deallocations can have a negative performance impact. This could be due to:
  * an application performing many different queries (ORM, lists of varying length, ...)
  * some utility statements (SET application_name...).
    In this case, try to disable pg_stat_statements.track_utility.
  * pg_stat_statements.max is too low

This service uses the status file (see C<--status-file> parameter).

The C<--critical> and C<--warning> thresholds are optional.
Warning or critical will be raised if the number of deallocations
since the previous check exceeds the thresholds.
...
sub check_pg_stat_statements_dealloc {
    ...
    return status_unknown( $me, [ 'Ensure that the pg_stat_statements has been loaded in shared_preload_libraries.' ] ) if ($rs[0][1] ne 't');
    ...
    if ( $dealloc_delta < 0) {
        return status_unknown( $me, [ sprintf "Negative dealloc delta; did you call pg_stat_statements_reset()?: %d deallocs",$dealloc_delta], \@perfdata)
    }
...
sub check_replication_slots {
    ...
    if ( defined $args{'warning'} ) {
        my $thresholds_re = qr/(wal|spilled|remaining)\s*=\s*(?:([^,]+))/i;

        if ($args{'warning'} =~ m/^$thresholds_re(\s*,\s*$thresholds_re)*$/
            and $args{'critical'} =~ m/^$thresholds_re(\s*,\s*$thresholds_re)*$/
        ) {
            while ( $args{'warning'} =~ /$thresholds_re/g ) {
                my ($threshold, $value) = ($1, $2);
...
SELECT d.typname,
       ...
       pg_catalog.quote_ident(d.nspname) || '.' || pg_catalog.quote_ident(d.relname) || '.' || pg_catalog.quote_ident(d.attname)
FROM pg_class seq
LEFT JOIN (SELECT d.objid, t.typname, n.nspname, c.relname, a.attname
            ...
            WHERE d.classid='pg_catalog.pg_class'::pg_catalog.regclass
              ...
              AND d.deptype='a') d ON d.objid=seq.oid
WHERE seq.relkind='S'};
        ...
        {
            # Version 10.0 and higher we now have pg_sequence and functions to
...
SELECT
    ...
    pg_catalog.quote_ident(nspname) || '.' || pg_catalog.quote_ident(relname) || '.' || pg_catalog.quote_ident(attname),
    CASE
   ...
   LEFT JOIN (SELECT d.objid, nspname, relname, attname, typname
                FROM pg_catalog.pg_depend d
               INNER JOIN pg_catalog.pg_class c  ON c.oid=d.refobjid
               INNER JOIN pg_catalog.pg_namespace n ON n.oid=c.relnamespace
               INNER JOIN pg_catalog.pg_attribute a ON ( a.attrelid=c.oid
               ...
               WHERE d.classid='pg_catalog.pg_class'::pg_catalog.regclass
                 AND d.refclassid='pg_catalog.pg_class'::pg_catalog.regclass
                 AND d.deptype='a') class
     ON class.objid=seq.seqrelid;};
        ...
        foreach my $seq (keys %sequences)
        {
            ...
            # When we can't link the sequence to a column, we trust the dba/developer
            # and we're not going to try to guess.
            ...
            if ( defined $max_value ) {
                $real_max_value = ($max_val_seq <= $max_value) ? $max_val_seq : $max_value;
            } else {
                $real_max_value = $max_val_seq;
            }
        ...
        foreach my $seq(keys %sequences) {
            ...
            my $seq_desc;
            my $long_seq_desc;
            if ( $sequences{$seq}->{COLNAME} ne '' ) {
                $seq_desc="$db.$seq(" . $sequences{$seq}->{COLNAME} . ')';
                $long_seq_desc="$db.$seq(owned by " . $sequences{$seq}->{COLNAME} . ')';
            } else {
                $seq_desc="$db.$seq";
                $long_seq_desc="$db.$seq";
            }
            my $seq_criticity=0;
            ...
            if ($seq_criticity>=1) {
                ...
                $criticity = ($criticity>$seq_criticity) ? $criticity : $seq_criticity; # Take highest of the criticities
            }
        }
    }
...
sub check_streaming_delta {
    ...
    my %queries         = (
        ...
        #
        # TAP tests relies heavily on column *ORDER* (not column name)! Columns
        # 4, 5, 6 and 7 are read/changed from t/lib/Mocker/Streaming.pm during
        # TAP tests. See comments in t/lib/Mocker/Streaming.pm for more info.
        #
        # If you need to change the column order or logic, you must change it
        # in TAP tests and in Mocker::Streaming as well.
        #
        # 1st field: slot application_name
        # 2nd field: client_addr
        # 3rd field: pid
        # 4th field: sent_lsn    <= changed during TAP tests by Mocker::Streaming
        # 5th field: write_lsn   <= changed during TAP tests by Mocker::Streaming
        # 6th field: flush_lsn   <= changed during TAP tests by Mocker::Streaming
        # 7th field: replay_lsn
        # 8th field: current lsn <= read during TAP tests by Mocker::Streaming
    ...
    my %queries   = (
        ...
        $PG_VERSION_74 =>  q{
          SELECT current_database(), ns.nspname AS schemaname, tblname, bs*tblpages AS real_size,
            ...
            CASE WHEN tblpages > 0 AND tblpages - est_num_pages > 0
...
DB_LOOP: foreach my $stat (@rs) {
        ...
        $frate = 60 * $diff_number / (tv_interval $last_check, $now);
        $brate = 60 * $diff_size   / (tv_interval $last_check, $now);
...
# Make sure the path given as status file is a file, not a folder.
pod2usage(
    -message => "FATAL: --status-file must be a path to a file not a directory.\n"
    -exitval => 127
) if -d $args{'status-file'};
...
check_pgactivity version 2.8, released on Tue Oct 14 2025.
